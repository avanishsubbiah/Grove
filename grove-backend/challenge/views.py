...
def create_challenge(request):
    ...
    n = Notification(user_to=challengee,
                     content=f"{prompt}", src=f"{user.first_name} {user.last_name}")
    chal.save()
    n.save()
...
def get_active_challenges(request):
    ...
    all_chals = UserChallengeSerializer(
        chals, many=True).data + DailyChallengeSerializer(dailies, many=True).data
